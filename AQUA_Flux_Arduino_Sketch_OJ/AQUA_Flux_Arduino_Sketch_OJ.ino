...
{
  Serial.begin(9600);
  delay(3000); //3 second delay before wire begin
  Wire.begin ();
...
void loop(){
 ...
 // Watchdog.reset();
  ...
  delay(50);
  // OJ Addition - to prevent issue of SCL being held low, recover the I2C bus 
  int co2Value = readCO2();
  bool hadToReset=false;
  if(co2Value == 0){
    I2CRecoverBus();
    hadToReset=true;
    delay(3000);
...
void I2CRecoverBus()
{
  ...
  // If SCL LOW → device stuck → cannot recover
  if (digitalRead(A5) == LOW) {
    Serial.println("SCL held LOW by device");
    return;
  }

  pinMode(A5, OUTPUT);

  // clock out stuck slave
  for (int i = 0; i < 20; i++) {
    digitalWrite(A5, LOW);
